...
The method described here probably constitutes a simple type of unsupervised [machine learning](https://en.wikipedia.org/wiki/Machine_learning). There are actually two methods for computing genetic distances underpinning our method, and these can be used together as an ensemble or independantly, to compute genetic distances for subsequently heirarcical clustering. The method requires MLST datasets as input represented in the form of a Haplotype Data Sheet or HDS. An example of this format is provided in this repository. These two genetic distance computation methods (Plucinski's Bayesian method and Barratt's heuristic definition of genetic distance) were developed to address four main issues relating to the analysis of massive and complex MLST datasets:
